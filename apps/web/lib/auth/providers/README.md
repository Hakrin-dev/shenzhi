...
`apps/web/lib/auth/email/callbacks.ts` and `apps/web/lib/auth/email/messages.ts` prepare the
Better Auth callback boundary and message content. `apps/web/lib/auth/server.ts` only
