...
function cited(children: ReactNode) {
  return Children.map(children, (child) => typeof child === "string" ? citeText(child) : child);
}
...
export function MarkdownContent({ text }: { text: string }) {
  return <div className="min-w-0 break-words text-sm leading-7">
    <ReactMarkdown remarkPlugins={[remarkGfm, remarkMath]} rehypePlugins={[[rehypeKatex, { trust: false, strict: "ignore" }]]}
      components={{
        p: ({ children }) => <p className="my-2">{cited(children)}</p>,
        li: ({ children }) => <li>{cited(children)}</li>,
        ...
        strong: ({ children }) => <strong>{cited(children)}</strong>,
        ...
        th: ({ children }) => <th className="border border-line bg-sidebar px-3 py-2 text-left">{cited(children)}</th>,
        td: ({ children }) => <td className="border border-line px-3 py-2">{cited(children)}</td>,
