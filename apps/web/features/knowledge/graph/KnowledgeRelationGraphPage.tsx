...
/** 论文关系图谱页 `/knowledge/search/[paperId]/graph` —— 三栏图谱工作台 */
export function KnowledgeRelationGraphPage({ paperId }: { paperId: string }) {
  return (
    <AppShell>
      <div className="flex h-[calc(100dvh-3.5rem)] flex-col bg-background lg:h-screen">
        {/* key=paperId：切换中心论文时重挂载，重置选择 / 方向 / 布局状态 */}
        <KnowledgeGraphWorkbench key={paperId} paperId={paperId} />
      </div>
    </AppShell>
  );
}
