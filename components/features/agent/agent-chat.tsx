...
import { AI_BACKEND_MODE, mapToAMode, mapToBStyle } from "@/lib/api/search";
import {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-primary/20 bg-primary-soft/40 dark:bg-card/60 dark:border-line/60">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full cursor-pointer items-center gap-2 px-3 py-2 text-left transition-colors hover:bg-primary-soft/60 dark:hover:bg-card/80"
      >
        <ChevronDown
          className={cn(
        ...
        {streaming ? (
          ...
          <span className="rounded-md bg-faint/10 px-1.5 py-0.5 text-[10px] font-medium text-muted">
            已完成 · {tokens} 字
          </span>
        )}
      ...
      >
        <div className="overflow-hidden">
          <div className="border-t border-primary/10 bg-white/50 p-3 dark:bg-panel/50 dark:border-line/40">
            <div
              className="max-h-[380px] overflow-auto whitespace-pre-wrap break-words rounded-lg bg-white/80 p-3 text-[12.5px] leading-6 text-ink-2 shadow-inner ring-1 ring-line/60 dark:bg-card dark:text-ink-2 dark:ring-line/40"
            >
              {content}
              {streaming && (
...
export function AgentChat() {
  /* ---------- 0. 演示模式自动登录（需同时配置 NEXT_PUBLIC_DEMO_MODE=true 和 DEMO_USER/DEMO_PASS） ---------- */
  ...
  // 同步 NextAuth session 状态到本地 auth store（保证侧边栏用户卡片显示正确）
  useEffect(() => {
    if (authStatus === "authenticated" && authSession?.user?.name) {
      authLogin();
      // 如果 store 里的名字是默认的，用 session 里的真实名字覆盖
      const currentName = useAuthStore.getState().userName;
      if (!currentName || currentName === "演示用户") {
        useAuthStore.setState({ userName: authSession.user.name });
      }
    } else if (authStatus === "unauthenticated") {
      authLogout();
    }
  }, [authStatus, authSession, authLogin, authLogout]);

  // 演示模式：未登录时自动以演示账号静默登录（需后端 route 配合 DEMO_PASS 环境变量）
  useEffect(() => {
    if (!demoMode) return;
    ...
    nextAuthSignIn("credentials", {
      name: demoUser,
      password: "demo-mode", // 实际校验由服务端 DEMO_PASS 环境变量控制；此处仅为占位
      redirect: false,
    }).catch(() => {});
  }, [authStatus, demoMode, demoUser]);
  ...
  const patchAssistantByBackendId = useCallback(
    (backendId: string, patch: Partial<ChatUIMessage>) => {
      ...
      setMessages((prev) => {
        ...
        // meta 增量合并：剔除 patch 中值为 undefined 的键。
        // 展开运算符会让 undefined 覆盖已有值（如后续 meta 事件的 warnings: undefined
        // 会抹掉先前注入的联网搜索失败/附件截断警告）
        if (patch.meta) {
          const clean: Record<string, unknown> = {};
          for (const [k, v] of Object.entries(patch.meta)) {
            if (v !== undefined) clean[k] = v;
          }
          patch.meta = { ...(m.meta ?? {}), ...clean };
        }
  ...
  const sendInternal = useCallback(
    async (text: string, override?: { resumeLastMsgId?: string }) => {
      ...
      // 引用式守卫：双击发送/双 Enter 时 state 闭包可能仍是旧值（isStreaming=false），
      // 用 messagesRef 同步判断，避免第二条 send 把第一条流式中断（表现为 ERR_ABORTED）
  ...
  const resumeLast = useCallback(async () => {
    ...
    // 模式 B 的 resume 是整条重新生成（/api/ai/chat 无状态）→ 清空残文避免拼接重复；
    // 模式 A 是断点续传 → 保留已生成内容
    const clearContent = AI_BACKEND_MODE === "B";
    if (idx !== undefined) {
      setMessages((prev) => {
        const next = prev.slice();
        if (next[idx])
  ...
  /* ---------- 11.5 重新生成：指定位置的 AI 消息重生成 ---------- */
  const regenerateAtIndex = useCallback(
    (index: number) => {
      const msgs = messagesRef.current;
      if (index <= 0 || index >= msgs.length) return;
      const aiMsg = msgs[index];
      if (aiMsg.role !== "assistant") return;
      // 找到对应用户消息（前一条 user 消息）
      let userMsg = null;
      ...
      // 重新发送
      window.setTimeout(() => sendInternal(userMsg.content), 0);
    },
    [sendInternal],
  );
  ...
  if (messages.length === 0) {
    return (
      // ⚠️ 必须与下方对话视图同样包裹 CWebSearchProvider：
      // 该 Provider 在 mount 时向 composer store 注册 webSearchFn。
      // 若空状态不挂载，新对话的第一条消息发送时 webSearchFn 为 null，
      // 联网开关开了也会被静默跳过（第二句起才生效）。
      <CWebSearchProvider>
      <div className="flex">
        {historyPanel}
        <div className="flex min-h-screen min-w-0 flex-1 flex-col items-center justify-center gap-6 px-4 sm:px-6">
    ...
    <CWebSearchProvider>
      ...
      <div className="flex h-screen min-w-0 flex-1 flex-col">
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className="mx-auto max-w-5xl space-y-4 px-4 py-6 sm:space-y-6 sm:px-6 sm:py-8">
            {messages.map((msg, i) => {
              if (msg.role === "user") {
                return (
                  <div key={msg.id} className="group flex justify-end">
                    <div className="relative max-w-[80%]">
                      <p className="rounded-2xl rounded-br-md bg-primary px-4 py-2.5 text-[15px] leading-relaxed text-white">
                        {msg.content}
                      </p>
                      <button
                        type="button"
                        title="编辑并重发"
                        onClick={() => {
                          setComposerMessage(msg.content);
                          // 删除这条及之后的消息，让用户重新发送
                          setMessages((prev) => {
                        ...
                        className="absolute -left-8 top-1/2 -translate-y-1/2 rounded-md p-1 text-faint opacity-0 transition-opacity hover:bg-white/10 hover:text-white group-hover:opacity-100"
                      >
                  ...
                  <div className="max-w-[80%] space-y-3">
                    ...
                    {msg.status === "error" ? (
                      ...
                      <div className="overflow-hidden rounded-2xl border border-line/60 bg-card shadow-card">
                        ...
                        {/* 操作栏：复制 + 重新生成（非流式、非错误态才显示；错误态已由上方三元排除） */}
                        {msg.status !== "streaming" && msg.status !== "sending" && msg.content && (
                          <div className="flex items-center gap-1 border-t border-line/40 px-3 py-1.5">
                            <button
                              type="button"
                              title="复制全文"
                              onClick={() => copyMessageContent(msg.content)}
                              className="flex items-center gap-1.5 rounded-md px-2 py-1 text-[12px] text-faint transition-colors hover:bg-chip hover:text-ink-2"
                            >
