...
export function SessionList({
  ...
  const handleFavorite = async (s: SessionListItem, e: React.MouseEvent) => {
    e.stopPropagation();
    // 乐观更新
    const next = !s.isFavorite;
    setSessions((prev) => {
      const updated = prev.map((x) =>
        x.id === s.id ? { ...x, isFavorite: next } : x,
      );
      // 收藏的排前面
...
function SessionGroups({
  ...
  onSelect: (s: SessionListItem) => void;
  onStartEdit: (s: SessionListItem) => void;
  onCommitEdit: (id: string) => void;
  onDelete: (s: SessionListItem) => void;
  onFavorite: (s: SessionListItem, e: React.MouseEvent) => void;
}) {
  const favs = sessions.filter((s) => s.isFavorite);
  const others = sessions.filter((s) => !s.isFavorite);

  return (
    <div className="flex flex-col gap-0.5">
      {favs.length > 0 && (
        <>
          <p className="shrink-0 px-1 pb-1 pt-2 text-[11px] font-medium tracking-wide text-faint">
            ⭐ 收藏
          </p>
          {favs.map((s) => (
            <SessionItem
      ...
      {editingId === s.id ? (
        ...
        <>
          ...
          <button
            ...
            onClick={(e) => onFavorite(s, e)}
            className={cn(
              "mr-1 shrink-0 rounded p-0.5 transition-all",
              s.isFavorite
                ? "text-amber-500 opacity-100"
                : "text-faint opacity-0 group-hover:opacity-100 hover:text-amber-500",
            )}
          >
            <Star
              className="size-3.5"
