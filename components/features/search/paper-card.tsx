 ...
 *   —— 从 A 模块替换 B 旧版 PaperCard，新增 layout?: "feed" | "explore" prop
 *   —— layout="explore"：搜索结果页使用，隐藏 200px 缩略图列 + 「立即阅读」悬浮按钮，
 *     只保留纯 Feed 样式的论文摘要与 AI 解读入口
 ...
 * UPDATE: 2026-08-18 A+B 单前端整合 —— 新增 layout prop（默认 "feed" / 搜索结果页传 "explore"）
 */
export function PaperCard({
  ...
  paper: FeedPaper;
  index: number;
  /** feed = 首页发现流（含缩略图+立即阅读按钮） / explore = 搜索结果页（纯文本布局） */
  layout?: "feed" | "explore";
}) {
