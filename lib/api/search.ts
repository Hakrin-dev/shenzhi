...
export const FALLBACK_SEARCH_CONFIG: SearchConfig = {
  models: [
    // 全部为百炼（DashScope）真实存在的模型，已逐一实测可用。
    // ⚠️ 旧配置含 deepseek-chat（DeepSeek 官方模型名），百炼上不存在 → 404 model_not_found
    { value: "default", label: "默认", enabled: true },
    { value: "qwen-max", label: "Qwen Max · 旗舰", enabled: true },
    { value: "qwen-turbo", label: "Qwen Turbo · 极速", enabled: true },
    { value: "qwen3-max", label: "Qwen3 Max · 深度思考", enabled: true },
    { value: "deepseek-r1", label: "DeepSeek R1 · 推理", enabled: true },
...
async function streamB(
  ...
  try {
      ...
      }),
      {
        ...
        onEvent: (_eventName, dataStr) => {
          ...
          switch (ev.type) {
            ...
            case "error":
              // 保留服务端原始错误码（字符串如 model_not_found / SCHEMA_ERROR）。
              // 旧写法 Number(ev.code) || 20004 会把一切字符串码吞成 20004「生成中断」，
              // 掩盖真实故障原因（如模型名不存在、Key 无效）。
              safe.onError?.({
                code: String(ev.code ?? 20004),
    ...
    // 非 2xx 时 readSSEStream 抛出的 message 是原始 SSE 文本（data: {"type":"error",...}），
    // 尝试提取真实 code/message；纯网络中断（无 JSON）才归为 20004「生成中断」
    ...
    if (m) {
      try {
        ...
        /* 非 JSON 保持 20004 + 原始文本 */
      }
    }
    safe.onError?.({ code, message });
  }
}
