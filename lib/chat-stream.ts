...
export interface SendOptions {
  ...
  /** 本次发送实际搜到的联网结果（send 内部写入，resume 重建 payload 时复用） */
...
export function useAskSession(getCbs: () => AskStreamCallbacks): AskSessionHandle {
    ...
    async (opts: SendOptions): Promise<string> => {
      ...
      if (opts.webSearch && opts.webSearchFn) {
        try {
          ...
          // 搜索失败不中断对话，但必须让用户看见原因（Key 无效等），
          // 在拿到 backendMsgId 后作为 meta warning 注入思考面板 ⚠️
          webSearchWarning = `联网搜索失败：${(e as Error).message}`;
      ...
      // 同时写入 send 快照，resume（继续生成）重建 payload 时可复用同一份搜索上下文
      lastSendOptsRef.current = { ...opts, webSearchSources };
      ...
      // 联网搜索失败（Key 无效 / 超时等）：注入思考面板 ⚠️（与附件截断告警同一通道），
      // 避免用户开着联网开关却无来源卡片、也无任何失败提示。
      // 形状必须是 { code, message } 对象（ThinkingPanel 渲染 w.code / w.message）
      if (webSearchWarning) {
        cbs().onMeta(backendMsgId, {
          phase: "warning" as any,
          read_count: undefined as any,
          context_truncated: false,
          warning: { code: "WEB_SEARCH_FAILED", message: webSearchWarning },
        } as any);
      }
      ...
      try {
        ...
        if (aborter.signal.aborted) {
          // 主动中断（Stop 按钮 / 新对话切换）：不能直接 return ——
          // UI 层的落库逻辑挂在 onDone 里，跳过会导致该轮 AI 回答
          // （含已流式生成的部分）完全不落库，回看会话只剩提问没有回答。
          // 以 stopped 状态触发 onDone，UI 层会把已生成的部分内容写库。
          clearTokenTimer();
          if (gotAnyDelta) {
            cbs().onDone(backendMsgId, {
  ...
  const resumeLast = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      // 模式 B：resumeChatMessage 只回传原 id，而 /api/ai/chat 是无状态 POST，
      // 必须用上次 send 的快照重建 modeBPayload（否则 streamB 报 20004 缺少 modeBPayload）
      const opts = lastSendOptsRef.current;
      const modeBPayload = opts
        ? {
            ...
            // 语义为重新生成整条回答；history 末尾已含当前提问。
            // webSearchSources 用 send 时的快照，保持联网上下文一致
            messages: buildModelMessages({
              style: opts.style,
              history: opts.history,
              attachments: opts.attachments,
              webSearchSources: opts.webSearchSources,
            }).messages,
          }
        : undefined;
      ...
      await streamChatMessage(newId, rh, modeBPayload, { signal: aborter.signal });
      return newId;
    } catch (e) {
