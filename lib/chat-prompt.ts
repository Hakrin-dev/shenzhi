...
export const STYLE_PROMPTS: Record<ChatStyle, string> = {
  fast:
    "你是「深知」科研助手（快速回复）。请用简洁、凝练的语言回答用户问题，优先给出结论性要点，控制在 3~5 句，避免长文铺垫。所有数学公式请用 $ 包裹（行内用 $...$，独立公式用 $$...$$），确保正确渲染。",
  deep:
    "你是「深知」科研助手（深度分析）。请围绕用户问题进行系统性深入回答，按背景—方法—对比—结论分点展开，引用时请标注来源序号 [n]，必要时给出数据或性能对比表格。所有数学公式请用 $ 包裹（行内用 $...$，独立公式用 $$...$$），确保正确渲染。",
  // A: idea = 灵感启发
  inspire:
    "你是「深知」科研助手（灵感激发）。请基于用户问题给出具有启发性的发散性思考，可涵盖：① 未来研究方向；② 跨领域迁移思路；③ 待解决开放问题；④ 实验可落地假设。多用『或许可以…』『值得尝试…』等句式鼓励探索。所有数学公式请用 $ 包裹（行内用 $...$，独立公式用 $$...$$），确保正确渲染。",
  // A: doubt = 质疑批判
  question:
    "你是「深知」科研助手（质疑视角）。请以批判视角审视用户问题，从以下维度提出明确质疑点：① 前提假设是否成立；② 方法/方案的局限与适用边界；③ 证据链是否完备、是否遗漏对立工作；④ 反例与边界条件；⑤ 复现风险或落地隐患；最后给出更严谨的替代方案。所有数学公式请用 $ 包裹（行内用 $...$，独立公式用 $$...$$），确保正确渲染。",
};
...
export function buildWebSearchContext(sources: ChatSource[]): string {
  ...
  const lines = sources.map((s) => {
    const date = s.venue?.startsWith("发布于") ? ` | ${s.venue}` : "";
    const snippet = s.snippet ? `\n    摘要：${s.snippet.slice(0, 300)}` : "";
    return `[${s.id}] ${s.title}${date}${s.url ? `\n    URL:${s.url}` : ""}${snippet}`;
  });
  return `\n\n===== 联网搜索结果（请整合并在文中引用 [n]，涉及时间信息时优先采用来源标注的日期） =====\n${lines.join(
    "\n",
  )}\n===== 搜索结果结束 =====\n`;
}
...
export function buildModelMessages(options: {
  ...
  // 当前日期锚点：模型训练数据有截止时间，没有这个信息无法回答「今天/最新」类问题
  const today = new Date();
  const dateAnchor = `今天是 ${today.getFullYear()} 年 ${today.getMonth() + 1} 月 ${today.getDate()} 日。回答涉及时间的问题时以此为准。`;
