...
/* =========================================================
 *  模型真实接入配置（OpenAI 兼容协议）
 *  优先读取 DASHSCOPE_*（阿里云百炼），回落 DEEPSEEK_*（DeepSeek 官方）
...
> {
  ...
  if (!DEEPSEEK_API_KEY) {
    yield {
      ...
      message:
        "未读取到 DASHSCOPE_API_KEY（百炼）或 DEEPSEEK_API_KEY。请检查 .env.local 是否配置了正确的 Key，并重启 Next.js 服务（环境变量仅在启动时加载）。",
    };
    return;
  }
  ...
  // 推理/思考型模型：deepseek-r1、*-reasoner、qwen3 系列（默认输出思维链）
  const isReasoningModel = /reasoner|r1|qwen3/i.test(model);
  ...
  if (!signal.aborted && fullContent.length > 20) {
    try {
      const followupModel = DEEPSEEK_DEFAULT_MODEL;
